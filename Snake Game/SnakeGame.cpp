...
int minusButtonClickTime = 0;
bool minusButtonClicked = false;

int plusButtonClickTime = 0;
...
SnakeGame::SnakeGame() {
	...
	int startButtonY = buttonAreaCenterY - buttonHeight / 2 - 225;
	...
	int minusButtonX = buttonAreaCenterX - buttonWidth / 2;  // additional
	int minusButtonY = infoButtonRect.y + infoButtonRect.h + 120;
	minusButtonRect = { COLS * TILE_SIZE + 50, ROWS * TILE_SIZE / 2 + 300, 100, 100 };

	int plusButtonX = buttonAreaCenterX + buttonWidth / 2 - 100; // additional
	int plusButtonY = infoButtonRect.y + infoButtonRect.h + 75;
	plusButtonRect = { plusButtonX, plusButtonY, 100, 100 };
	...
	while (SDL_PollEvent(&e)) {
		if (e.type == SDL_QUIT) {
			...
			if (e.button.button == SDL_BUTTON_LEFT) {
				...
				//additional
				if (mouseX >= minusButtonRect.x && mouseX <= minusButtonRect.x + minusButtonRect.w && mouseY >= minusButtonRect.y && mouseY <= minusButtonRect.y + minusButtonRect.h) {

					if (musicVolume > 0) {
						if (musicVolume < Mix_VolumeMusic(0)) {
							musicVolume = 0;
						}
						musicVolume -= 10; 
						Mix_VolumeMusic(musicVolume);
						minusButtonClicked = true;
						minusButtonClickTime = SDL_GetTicks();
						soundManager.playClickSound("C:/Users/User/Desktop/Mp3's/click.wav");
					}
				}
				else if (mouseX >= plusButtonRect.x && mouseX <= plusButtonRect.x + plusButtonRect.w && mouseY >= plusButtonRect.y && mouseY <= plusButtonRect.y + plusButtonRect.h) {
					if (musicVolume < 128) {
						if (musicVolume > Mix_VolumeMusic(130)) {
							musicVolume = 130;
						}
						musicVolume += 10;
						Mix_VolumeMusic(musicVolume);
						plusButtonClicked = true;
						plusButtonClickTime = SDL_GetTicks();
						soundManager.playClickSound("C:/Users/User/Desktop/Mp3's/click.wav");
					}
				}
				...
				if (mouseX >= startButtonRect.x && mouseX <= startButtonRect.x + startButtonRect.w && mouseY >= startButtonRect.y && mouseY <= startButtonRect.y + startButtonRect.h) {
					...
					soundManager.playClickSound("C:/Users/User/Desktop/Mp3's/click.wav"); // additional sound for info button
				}
			}
		}
...
void SnakeGame::renderButtons() {
	...
	int minusButtonDuration = currentTime - minusButtonClickTime;
	if (minusButtonDuration < 200) {
		SDL_SetRenderDrawColor(renderer, 135, 206, 250, 255);
	}
	else {
		SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255);
	}
	SDL_RenderFillRect(renderer, &minusButtonRect);
	renderText("-", minusButtonRect.x + 40, minusButtonRect.y + 10, textColor);

	int plusButtonDuration = currentTime - plusButtonClickTime;
	if (plusButtonDuration < 200) {
		SDL_SetRenderDrawColor(renderer, 255, 165, 0, 255);
	}
	else {
		SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255);
	}
	SDL_RenderFillRect(renderer, &plusButtonRect);
	renderText("+", plusButtonRect.x + 35, plusButtonRect.y + 10, textColor);
}
...
void SnakeGame::render() {
	...
	string speedText = "Speed: " + to_string(speed);
	SDL_Surface* speedSurface = TTF_RenderText_Blended(font, speedText.c_str(), greenColor);
	SDL_Texture* speedTexture = SDL_CreateTextureFromSurface(renderer, speedSurface);
	SDL_Rect speedRect = { COLS * TILE_SIZE + 10, ROWS * TILE_SIZE / 2 - 75, speedSurface->w, speedSurface->h };
	SDL_RenderCopy(renderer, speedTexture, NULL, &speedRect);
	SDL_FreeSurface(speedSurface);
	SDL_DestroyTexture(speedTexture);

	string volumeText = "" + to_string(musicVolume);
	SDL_Surface* volumeSurface = TTF_RenderText_Blended(font, volumeText.c_str(), greenColor);
	SDL_Texture* volumeTexture = SDL_CreateTextureFromSurface(renderer, volumeSurface);
	SDL_Rect volumeRect = { COLS * TILE_SIZE + 150 , ROWS * TILE_SIZE - 150 , speedSurface->w, speedSurface->h };
